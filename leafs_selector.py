...
if __name__ == '__main__':
    pass
    # берем два экг: эталонную и вторую.
    # 1) На обеих делаем одно и то же предсказание, что сематически точка поставится верно не только на эталоне

    # 2) если ок, то дальше латеральное торможение для первой (родительской) точки

    # 3)когда генерация листов (т.е.распознавание на 1 шаг) работает,
    # и торможение первой верщины работает - вот тогда можно писать recognise (для вручную заданной программы)
    # 4) А затем уже рост/релаксация: там понабодится обратное распространение R  [ТУТ ВЫСТУПЛЕНИЕ]
    # 5) и затем пересчет бассейнов при придесказании в уже накрытую описанием область.
